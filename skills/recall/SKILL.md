...
allowed-tools: mcp__arcana__arcana_recall, mcp__arcana__arcana_timeline, mcp__arcana__arcana_search
---
...
```
arcana_recall({ entity: "<entity name>" })
```
...
```
arcana_timeline({ today: true })
arcana_timeline({ week: true })
arcana_timeline({ search: "<keywords>" })
```

**For semantic search** (broader knowledge — anything not tied to a single entity):
```
arcana_search({ query: "<query>" })
```
...
```
arcana_recall({ entity: "dashboard redesign" })
arcana_timeline({ search: "dashboard" })
```

**User references a past decision:** "Why did we go with PostgreSQL?"
```
arcana_recall({ entity: "PostgreSQL" })
arcana_search({ query: "PostgreSQL decision rationale" })
```

**User asks "what do you know about me?"** Combine multiple lookups:
1. `arcana_brain_stats({})` — scope of the workspace
2. `arcana_search({ query: "user preferences role background" })`
3. `arcana_timeline({ week: true })`
